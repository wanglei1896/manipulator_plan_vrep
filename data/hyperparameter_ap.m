...
function result = hyperparameter_ap()
    ...
    result.ap1_to1=0.001; %fdt(偏离目标值)代价与oa(避障部分)的混合比例，,越大oa比例越高，tradeoff
    result.ap1_to2=0; %前两者(fdt和oa)与fq(前一位姿与后一位姿距离)的混合比例，越大fq比例越高，tradeoff
    ...
    result.ob_e=1e-8; %避障部分的最小精度，低于此最小距离则代价不再增长
    result.ob_mindis=1e-2; %避障部分最小距离
